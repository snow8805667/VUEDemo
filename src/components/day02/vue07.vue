  ...
  <div id="vue">
    ...
    <div>
      <fieldset>
        ...
        <br>年龄：
        <input type="number" name="Age" v-model.number="Age" placeholder="请输入年龄">
        <br>电话：
        <input
          type="number"
          name="telphone"
          v-model.number="telphone"
          placeholder="请输入电话"
          maxlength="11"
        >
        <br>
      ...
      <fieldset>
        ...
        <table>
          ...
          <tbody v-for="(item,index) in goods">
            ...
            <td>{{item.price}}</td>
            <td>
              <input type="button" value="-" @click="subGood(item);" :disabled="item.mount===1">
              {{item.mount}}
              <input type="button" value="+" @click="addGood(item);">
            </td>
            <td>
              <input type="button" value="删除" v-on:click="deletegood(index);">
            </td>
          </tbody>
        </table>
        <p>总价：{{total}}</p>
